...
void Help_Show()
{
	...
	while(loop_)
	{
		...
		case '?':
			...
			printsub("<던전의 탐색>                                      $ - 현재 가지고 있는 돈(미구현)",true,CL_normal);
			printsub("5   - 100턴 휴식                                   % - 자세한 캐릭터 정보",true,CL_normal);
			printsub("x   - 근처 조사                                    ^ - 신앙 정보",true,CL_normal);
			...
			printsub("d      - 아이템버리기                              <비게임 커맨드>",true,CL_normal);
