...
void Help_Show()
{
	...
	while(loop_)
	{
		...
		case '?':
			...
			printsub("x      - 근처 조사                                 \\ - 식별된 아이템 정보",true,CL_normal);
			printsub("s또는. - 1턴 넘기기                                A - 캐릭터 특성 확인",true,CL_normal);
			printsub("X      - 맵을 조사                                 [ - 입고있는 장비 표시",true,CL_normal);
			printsub("<,>    - 계단 오르기/내려가기                      } - 현재 무기 표시",true,CL_normal);
			printsub("O,C    - 문 열기/닫기                              \" - 현재 장신구 표시",true,CL_normal);
			...
			printsub("a - 권능 혹은 발동능력사용                         ",true,CL_normal);
			changedisplay(DT_SUB_TEXT);
			ReleaseMutex(mutx);
			break;
		case ':':
