...
bool item::draw(shared_ptr<DirectX::SpriteBatch> pSprite, shared_ptr<DirectX::SpriteFont> pfont, float x_, float y_, float scale_, int alpha_)
{
	if (image) {
		bool return_ = image->draw(pSprite, x_, y_, 0.0f, scale_, scale_, alpha_);
		if (type == ITM_POTION) {
			if (iden_list.potion_list[value1].iden) {
				img_item_potion_kind[min(PT_MAX-1,max(0,value1))].draw(pSprite, x_, y_, 0.0f, scale_, scale_, alpha_);
			}
			...
			if (iden_list.amulet_list[value1].iden) {
				img_item_amulet_kind[min(AMT_MAX - 1, max(0, value1))].draw(pSprite, x_, y_, 0.0f, scale_, scale_, alpha_);
			}
		}
