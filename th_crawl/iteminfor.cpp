	...
	case ITM_AMULET:
		{
			...
			text_ += "끼고 있는 채로 경험치를 얻으면 부적의 충전량이 점점 충전되며 100%가 되면 힘을 발휘한다.\n";
			text_ += "만약 끼고있는 부적을 벗으면 그 즉시 충전량이 0%로 떨어지므로 주의해야한다.\n";
			if(iden_list.amulet_list[it->value1].iden == 2)
			{
				...
				case AMT_PERFECT:
					...
					text_ += "100%가 된 부적을 낀채로 죽게되면 체력이 전부 회복되어 부활한다.\n";
					text_ += "단, 이 방법으로 부활하게되면 부적은 불타 사라지게된다.\n";
					break;
				case AMT_BLOSSOM:
					text_ = "춘설이변에 쓰인 봄의 기운을 모아서 특수한 결계를 발동시킬 수 있는 부적.\n";
					text_ += "끼고 있으면 주변의 봄의 기운이 모이고 충전이 되면 영격을 발동할 수 있게 된다.\n";
					text_ += "100%가 된 부적을 낀채로 v를 누르면 영격이 발동되어 주변의 적을 일정시간 마비시킨다.\n";
					text_ += "발동후에는 충전치가 0%가 되어 다시 충전할 수 있게 된다.\n";
					break;
				case AMT_TIMES:
					text_ = "영야이변을 해결할때 사용된 밤을 멈추는 주술의 힘이 담긴 부적.\n";
					text_ += "끼고 있으면 주변의 각부가 모이고 충전이 되면 주변의 시간의 흐름보다 빠르게 움직일 수 있다.\n";
					text_ += "100%가 된 부적을 낀채로 v를 누르면 가속이 발동한다.\n";
					text_ += "발동후에는 충전치가 0%가 되어 다시 충전할 수 있게 된다.\n";
					break;
				case AMT_FAITH:
					text_ = "신앙의 힘을 모을 수 있는 부적.\n";
					text_ += "끼고 있으면 주변의 신앙이 모이고 충전이 되면 신들과의 관계가 좋아진다.\n";
				...
				case AMT_WAVE:
					...
					text_ += "끼고 있으면 전파수신감도가 점점 좋아지고 충전이 되면 현재 층을 한 눈에 볼 수 있다.\n";
					text_ += "100%가 된 부적을 낀채로 v를 누르면 현재 층의 지형을 비추게 된다.\n";
					break;
				case AMT_SPIRIT:
					text_ = "몽전대사묘의 태자를 부활시킬때도 사용되었다던 신령을 담을 수 있는 부적.\n";
					text_ += "끼고 있으면 주변의 신령이 모이고 충전이 되면 가진 스펠카드에 신령을 불어넣을 수 있다.\n";
					text_ += "100%가 된 부적을 낀채로 v를 누르면 가지고 있는 스펠카드 1장을 충전할 수 있다.\n";
					break;
				case AMT_GRAZE:
					text_ = "어느 이변에서나 중요한 그레이즈의 힘이 담긴 부적.\n";
					text_ += "끼고 있으면 점차 충전이 되고 충전이 끝나면 당신은 탄막괴물이 된다.\n";
					text_ += "100%가 된 부적을 낀채로도 상시 그레이즈가 발동되어 원거리 탄막을 일정확률로 피할 수 있고\n";
				...
				case AMT_WEATHER:
					...
					text_ += "끼고 있으면 주변의 기질 구슬을 흡수하고 충전이 끝나면 부적에 숨겨있던 기질이 발동된다.\n";
					text_ += "100%가 된 부적을 낀채로 있으면 체력 회복속도가 약간 증가하며\n";
					text_ += "100%상태에서 v를 누르면 체력을 약간 회복할 수 있다.\n";
					break;
				case AMT_POPULAR:
					text_ = "종교가들이 주변의 인기를 모으기위해 만들었다던 부적.\n";
					text_ += "끼고 있으면 인기가 점점 높아지고 충전이 끝나면 ???\n";
					text_ += "100%상태에서 v를 누르면 ???\n";
					break;
				case AMT_OCCULT:
					text_ = "도시전설을 실체화하는 힘을 가진 오컬트볼의 힘이 담긴 부적.\n";
					text_ += "끼고 있으면 오컬트의 기운을 흡수하고 충전이 끝나면 오컬트의 힘을 방출한다.\n";
					text_ += "100%상태에서 v를 누르면 시야내의 모든 적대적인 소환물을 추방하며\n";
					text_ += "자신에 우호적인 소환물을 회복시키며 일정시간 강화시킨다.\n";
					break;
				default:
