...
string GetItemInfor(item *it)
{
	...
	case ITM_POTION:
		{
			if(iden_list.potion_list[it->value1].iden)
			{
				...
				case PT_MAGIC:
					...
					break;
				case PT_LEVETATION:
					text_ += "마시면 일시적으로 부유상태가 되는 물약. 부유상태에서는 깊은물따위를 건널 수 있게된다.\n";
					text_ += "그러나 비행제어가 없으면 땅에 떨어진 물약, 계단으로 내려가는 것이 불가능해진다.\n";
					break;
				case PT_POWER:
	...
	case ITM_RING:
		{
			if(iden_list.ring_list[it->value1].iden == 2)
			{
				...
				case RGT_GRAZE:
					...
					break;
				case RGT_LEVITATION:
					text_ += "부유를 사용할 수 있는 반지. 착용하면 능력사용에서 부유발동이 생긴다.\n";
					text_ += "부유상태에서는 깊은물따위를 건널 수 있게된다.\n";
					text_ += "그러나 비행제어가 없으면 땅에 떨어진 아이템, 계단으로 내려가는 것이 불가능해진다.\n";
					text_ += "사용에는 발동스킬이 필요하며 P가 약간 소모된다.\n";
					break;
				case RGT_INVISIBLE:
	...
	case ITM_MISCELLANEOUS:
		...
		break;
	case ITM_GOAL:
		text_ += "축하드립니다. 이것은 룬입니다. \n";
		text_ += "제작자가 룬의 대체품을 아직 생각하지 못해서 도트도 대충찍었습니다.\n";
		text_ += "현재는 룬1개만 들고 탈출해도 클리어로 인정됩니다. 도전하실분은 몇개 더 먹어보시길\n";
		text_ += "아직 오브도 없잖아! 1룬 클리어라니! \n";
		break;
	default:
