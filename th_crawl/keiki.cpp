...
random_extraction<haniwa_abil_key> haniwa_abil::getAbleHaniwaAbils() {
    ...
    int abil1_ = you.god_value[GT_KEIKI][0];
    int abil2_ = you.god_value[GT_KEIKI][1];
    int current_cost_ = you.piety;

    if (current_cost_ >= 160) return result;

    auto has_abil = [&](haniwa_abil_key key) -> bool {
        int idx = static_cast<int>(key);
        return (idx < 32) ? ((abil1_ >> idx) & 1) : ((abil2_ >> (idx - 32)) & 1);
    };
