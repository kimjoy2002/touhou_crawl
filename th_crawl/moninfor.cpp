...
string GetMonsterInfor(monster *it)
{
	...
	if (it->dream) {
		...
		case MON_SEIGA:
			...
			break;
		case MON_CLOSE_DOOR:
			text_ += "신의 권능에 의해 단단하게 닫힌 문이다. \n";
			text_ += "불가사의하게 닫힌 모습에 적들은 단순한 문이 아님을 눈치채고 부수려할 것이다.\n";
			text_ += "그러나 문은 생각이상으로 단단하며, 당신의 강함에 따라서 문 또한 같이 강해질 것이다.\n";
			text_ += "기이하게 닫힌 이 문은 시간이 지나면 원래대로 평범한 문으로 돌아올 것이다.\n";
			break;
		case MON_MURASA:
		...
		case MON_YOUMU:
			...
			break;
		case MON_LYRICA:
			text_ += "프리즘리버 소령악단의 삼녀인 리리카 프리즘리버.\n";
			text_ += "악단 내에서 타악기를 담당하며 키보드를 손대지않고도 연주가 가능하다.\n";
			text_ += "그녀가 사용하는 키보드는 사람의 감정에 영향을 주진 못하지만 능숙한 기교로\n";
			text_ += "언니들보다 공격적인 전투가 가능하다.\n";
			break;
		case MON_MERLIN:
			text_ += "프리즘리버 소령악단의 차녀인 메를랑 프리즘리버.\n";
			text_ += "악단 내에서 관악기를 담당하며 트럼펫을 손대지않고도 연주가 가능하다.\n";
			text_ += "트럼펫에서 나는 음색은 뇌를 울리는 소리로 듣는 사람의 감정을 들뜨게 만든다.\n";
			break;
		case MON_LUNASA:
			text_ += "프리즘리버 소령악단의 장녀인 루나사 프리즘리버.\n";
			text_ += "악단 내에서 현악기를 담당하며 바이올린을 손대지않고도 연주가 가능하다.\n";
			text_ += "바이올린에서 나오는 음색은 사람을 차분하게 만들며 오래 듣게 되면 상대의 감정을\n";
			text_ += "우울하게 만드는 것이 가능하다.\n";
			break;
		case MON_KOMACHI:
		...
		case MON_LARVA:
			...
			break;
		case MON_NEMUNO:
			text_ += "요괴의 산에 사는 야만보로 집단에 소속되기 싫어하여 혼자 살고있는 요괴.\n";
			text_ += "남과 엮이기 싫어하는 성격때문에 아무도 없는 곳에서 홀로 지내며 미개한 생활을 한다.\n";
			text_ += "그녀의 남과 교류하기 싫은 성격은 성역을 만드는 능력으로 표출되어 불가침영역을 만들어낸다.\n";
		...
		case MON_OCCULT_LONG:
			text_ += "키가 8척정도 되는 거대한 여자 귀신. 도시전설중 하나였지만 오컬트의 힘으로 실존하게 되었다.\n";
			text_ += "포포포포라는 이상한 소리를 내며 천천히 걸어온다. 몸집이 크기에 맷집도 상당하고 한방한방이 강하다.\n";
			break;
		case MON_OCCULT_DISK:
			text_ += "항상 모자라는 접시를 세는 귀신으로 도시전설중 하나였지만 오컬트의 힘으로 실존하게 되었다.\n";
			text_ += "근접전투에 강하진않지만 멀리서 접시를 던지면서 지원사격이 가능하다.\n";
			break;
		case MON_OCCULT_SMALL:
			text_ += "작은 초록색 소인으로 숲의 정령이라 불리며 도시전설중 하나였지만 오컬트의 힘으로 실존하게 되었다.\n";
			text_ += "작은 소인들은 각자의 힘은 약하지만 무리를 지어 빠르게 적을 공격한다.\n";
			break;
		case MON_KUNEKUNE:
			text_ += "정체모를 생명체로 이해하면 미쳐버린다는 도시전설중 하나였지만 오컬트의 힘으로 실존하게 되었다.\n";
			text_ += "제자리에서 꾸불꾸불거릴뿐 아무런 공격능력도 없지만 주변의 적들에게 지속적으로 혼란을 걸어버린다.\n";
			break;
		case MON_KANAME:
			...
			break;
		case MON_SHEEP:
			text_ += "꿈의 세계에서 살고 있는 양.\n";
			text_ += "얼핏보면 평범한 양과 다를게 없어보이지만 양에게 공격당할때마다 버틸 수 없는 졸음이 쏟아져온다.\n";
			text_ += "일정이상 졸음이 쌓이면 강제로 잠을 자게되며 잠에 빠진 동안엔 무방비 상태가 된다.\n";
			text_ += "잠든 상태에서 공격을 당한다면 잠에서 깨지만 양은 잠을 깨우지않고 꿈을 먹으면서 공격할 수 있다.\n";
			break;
		case MON_BEE:
			text_ += "\n";
			break;
		...
		case MON_SION:
			...
			break;
		case MON_CURSE_DOLL:
			text_ += "저주 받은 인형이다. 츠쿠모가미의 일종이다.\n";
			text_ += "종류마다 머리가 자란다던지 누가 보지않을때 움직인다던지 다양한 종류가 있는 듯 하다.\n";
			text_ += "인형인만큼 그렇게 강하진 않지만 무표정한 얼굴에서 섬뜩함이 느껴진다.\n";
			break;
		case MON_DANCING_WEAPON:
			text_ += "누군가 들고있던 무기가 힘을 얻은 츠구모가미\n";
			text_ += "단순히 쓰이던 도구에서 힘을 얻어서 하극상을 일으켜보려고 하는듯 하다.\n";
			text_ += "아직 힘이 약하기에 인간의 모습으론 변화하지못한듯하다.\n";
			break;
		case MON_DANCING_ARMOUR:
			text_ += "누군가 들고있던 옷이 힘을 얻은 츠구모가미\n";
			text_ += "단순히 쓰이던 도구에서 힘을 얻어서 하극상을 일으켜보려고 하는듯 하다.\n";
			text_ += "아직 힘이 약하기에 인간의 모습으론 변화하지못한듯하다.\n";
			break;
		case MON_ELEC_BALL:
		...
		case MON_BOMB:
			...
			break;
		case MON_DOREKING:
			text_ += "꿈의 세계의 관리인인 도레미의 진정한 모습(?)\n";
			text_ += "그 모습을 본 자는 살아남지 못한다는 전설이 있다.\n";
			break;
		case MON_CURIOSITY:
