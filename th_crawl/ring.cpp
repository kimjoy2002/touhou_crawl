...
ring_type goodbadring(int good_bad)
{
	if (good_bad == 1)
	{
		...
		ring_type list2_[9] = { RGT_STR, RGT_DEX, RGT_INT/*,RGT_GRAZE,RGT_LEVITATION*/,RGT_INVISIBLE,RGT_MANA,RGT_MAGACIAN,RGT_AC, RGT_EV, RGT_CONFUSE_RESIS};
		if(randA(2)>0)
			return list_[randA(5)];
		else 
			return list2_[randA(8)];
	}
	else if(good_bad>=0)
	{
		ring_type list_[15] = {RGT_STR, RGT_DEX, RGT_INT, /*RGT_FULL,*/RGT_POISON_RESIS,RGT_FIRE_RESIS,RGT_ICE_RESIS,
