...
bool seija_gift()
{
	...
	else
	{
		...
		printlog(LocalzationManager::formatString(joypadUtil::isUsingPad()?LOC_SYSTEM_GOD_SEIJA_GIFT_HELP_PAD:LOC_SYSTEM_GOD_SEIJA_GIFT_HELP, PlaceHolderHelper("a")),true,false,false,CL_help);
		MoreWait();
		...
		case GT_YUYUKO: //무작위 발동템 2개
			{
				...
				for (int i = 0; i < EVK_MAX; i++) {
					if (i != EVK_MAGIC_HAMMER && i != EVK_SKY_TORPEDO && i != EVK_CAMERA && i != EVK_FROZEN_FROG
						) {
						rand_.push(i);
					}
				}
