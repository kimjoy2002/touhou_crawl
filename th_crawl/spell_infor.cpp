...
string GetSkillInfor(skill_list skill)
{
	...
	case SKL_KANAKO_3:
		...
		text_ += "기도술에 비례하여 지속시간이 늘어나며 공격력은 늘어나지 않는다.\n";
		break;
	case SKL_GRAZE:
	...
	case SKL_EIRIN_0:
		...
		break;
	case SKL_EIRIN_1:
		text_ += "에이린에게 몸을 맡겨 원하는대로 몸을 조절한다. 원하는 스탯 하나를 다른 스탯으로 1만큼 옮길 수 있다.\n";
		text_ += "사용시 부작용으로 데미지와 일시적 스탯감소가 생긴다. 체력이 너무 낮을땐 사용할 수 없다.\n";
		text_ += "부작용의 데미지로 죽진 않지만 스탯감소로 죽을 수 도 있다.\n";
		break;
	case SKL_EIRIN_2:
		text_ += "순간적으로 체력과 마나를 한꺼번에 모두 채우는 에이린의 궁극 권능.\n";
		text_ += "효과는 강력하지만 사용후 체력과 마나의 최대치가 영구적으로 깍인다.\n";
		break;
	case SKL_BYAKUREN_1:
	...
	case SKL_SIZUHA_2:
		...
		text_ += "단풍방어구는 보통 방어구보다 높은 방어력, 낮은 패널티와 은밀보너스를 가지고 있다.\n";
		text_ += "단풍방어구는 착용,해제에 1턴밖에 들지 않고 경갑일수록 더욱 더 높은 은밀보너스를 주고\n";
		text_ += "중갑일수록 방어력이 높고 회피, 마법패널티가 더욱 높은 비중으로 줄어든다.\n";
		text_ += "그러나 이 단풍방어구는 당신의 화염저항을 1단계 낮추는 치명적인 단점을 가지고 있다.\n";
	...
	case SKL_YUKARI_3:
		...
		break;
	case SKL_YUKARI_4:
		text_ += "유카리가 경계를 건드려서 일정턴동안 현재 화면이 그대로 고정된다.\n";
