...
void wiz_mode()
{
	...
	case 'p':
		...
		break;
	case 'M':
		if(monster* mon_=BaseSummon(MON_FAIRY_SUN_FLOWER, 100, false, false, 2, &you, you.position, SKD_OTHER, -1))
		{
		...
		break;
	case 'm':
		if(monster* mon_=BaseSummon(MON_FAIRY_HERO, 100, false, false, 2, &you, you.position, SKD_OTHER, -1))
		{
			mon_->state.SetState(MS_SLEEP);
			mon_->flag &= ~M_FLAG_SUMMON;
			mon_->ReturnEnemy();
