...
void wiz_mode()
{
	...
	while(1)
	{
		...
		case '!':
			{
				you.Memorize(SPL_SUMMON_YOUKAI,true);
			}
			break;
		case 'M':
			if(monster* mon_=BaseSummon(MON_TOZIKO, 100, false, false, 2, &you, you.position, SKD_OTHER, -1))
			{
			...
			break;
		case 'm':
			if(monster* mon_=BaseSummon(MON_NAMAZ, 100, false, false, 2, &you, you.position, SKD_OTHER, -1))
			{
				mon_->state.SetState(MS_SLEEP);
				mon_->flag &= ~M_FLAG_SUMMON;
				mon_->ReturnEnemy();
