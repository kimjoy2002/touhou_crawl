...
void wiz_mode()
{
	...
	while(1)
	{
		...
		case 'm': 
			{
				  ...
				  if (monster* mon_ = BaseSummon(key_ == 'z' ? MON_MISTIA :MON_MURASA+ key_ - 'a', 100, false, false, 2, &you, you.position, SKD_OTHER, -1))
				  {
					  mon_->state.SetState(MS_SLEEP);
					  mon_->flag &= ~M_FLAG_SUMMON;
					  mon_->ReturnEnemy();
		...
		case 'M':
		{
			...
			if (monster* mon_ = BaseSummon(key_ == 'z' ? MON_BLOOD_HAUNT : MON_OCCULT_LONG + key_ - 'a', 100, false, false, 2, &you, you.position, SKD_OTHER, -1))
			{
				mon_->state.SetState(MS_SLEEP);
				mon_->flag &= ~M_FLAG_SUMMON;
				mon_->ReturnEnemy();
